...
class LRSelectGeneric:
    ...
    def func_oper_parsedate(self, value):
        ...
        date = parsedate(value, self.config.dayfirst)
        if not date:
    ...
    def func_oper_dateloc_to_lrstamp(self, value):
        ...
        for index, char in enumerate(value):
            if char.isnumeric():
                ...
                value = value[index:]
                break
        dtmod = date_to_lrstamp(value, self.config.dayfirst)
        if dtmod is None:
    ...
    def func_oper_dateutc_to_lrstamp(self, value):
        ...
        for index, char in enumerate(value):
            if char.isnumeric():
                ...
                value = value[index:]
                break
        dtmod = date_to_lrstamp(value, self.config.dayfirst, False)
        if not dtmod:
            raise LRSelectException('invalid date value on "datemod"')
        return oper, dtmod
    ...
    def select_generic(self, columns, criters, **kwargs):
        ...
        for token, data in lex.tokens:
            ...
            if (
                key == "distinct"
            ):  # specific key 'sort' for sql 'SELECT DISTINCT'
                select_type = _where
                prev_optoken = None
                continue
