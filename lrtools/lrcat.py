...
class LRCatDB:
    ...
    def __init__(
        ...
        def open_db(uri):
            try:
                ...
                (self.lrdb_version,) = self.cursor.execute(
                    'SELECT value FROM Adobe_variablesTable WHERE name="Adobe_DBVersion"'
                ).fetchone()
                log.info(
                    'LR database "%s" opened with uri "%s"',
                    self.lrcat_file,
                    uri,
                )
    ...
    def get_smartcoll_data(self, name_or_id, raw_value=False):
        ...
        try:
            ...
            self.cursor.execute(
                ...
                WHERE col.id_local=? AND cont.owningModule = "ag.library.smart_collection"',
                (pid,),
            )
        except ValueError:
            name = name_or_id
            self.cursor.execute(
                'SELECT cont.content FROM AgLibraryCollectionContent cont \
                JOIN AgLibraryCollection  col on col.id_local = cont.collection\
                WHERE  col.name LIKE ? AND cont.owningModule = "ag.library.smart_collection"',
                (name,),
            )
        try:
    ...
    def select_collections(self, what, collname=""):
        ...
        self.cursor.execute(
            f"SELECT id_local, name, creationId FROM AgLibraryCollection WHERE {where} ORDER BY name ASC"
        )
    ...
    def select_metadatas_to_archive(self):
        ...
        """
        return self.lrphoto.select_generic(
            "name=full", "metastatus=conflict, sort=capturetime"
        )
    ...
    def prodpath_from_xmp(self, xmp, prod_basepath, date_fmt):
        ...
        if datecapt:
            ...
            capturetime = datetime.strptime(
                "1900-01-01T00:00:00", "%Y-%m-%dT%H:%M:%S"
            )
        prodpath = os.path.normpath(
            os.path.join(str(prod_basepath), capturetime.strftime(date_fmt))
        )
