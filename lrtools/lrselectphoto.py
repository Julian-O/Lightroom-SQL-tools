...
class LRSelectPhoto(LRSelectGeneric):
    ...
    def __init__(self, config, lrdb):
        ...
        super().__init__(
            ...
            #
            {
                ...
                "datemod": {  # modif date (including keywords changes)
                    "True": ["i.touchtime AS datemod", None],
                },
                "datehist": {  # modif date based on history developement steps, ignoring exportations and publications (TODO: strings need to be localized )
                    "True": [
    ...
    def func_gps(self, value):
        ...
        if re_photo.match(value):
            ...
            try:
                (lat, lon), address = geocodage(self.config.geocoder, town)
                log.info(
            ...
            try:
                (lat1, lon1), address1 = geocodage(self.config.geocoder, town1)
                (lat2, lon2), address2 = geocodage(self.config.geocoder, town2)
                log.info(
